...
createDirectory(path.join(__dirname, OUT_DIR));

// Read all sets
let objectNames = readDir.readSync(IN_DIR, ['set*/'], readDir.INCLUDE_DIRECTORIES +
    readDir.CASE_SORT);
for (let j = 0; j < objectNames.length; j++) {
  let objectPath = path.join(__dirname, OUT_DIR, objectNames[j]);
  createDirectory(objectPath);

  let seqImagesPath = path.join(__dirname, IN_DIR, objectNames[j]);
